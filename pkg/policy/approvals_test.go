...
func TestApprovalEngineRequestApproveDeny(t *testing.T) {
	...
	if err := engine.Request(request); err != nil {
		t.Fatalf("Request returned error: %v", err)
	}
	if err := engine.Approve("approval-1", "operator", "ok"); err != nil {
...
func TestApprovalEngineDeny(t *testing.T) {
	engine := NewApprovalEngine()
	if err := engine.Request(ApprovalRequest{ID: "approval-2", Actor: "did:ia:user", Action: "capability.execute", Resource: "github:issue:create"}); err != nil {
		t.Fatalf("Request returned error: %v", err)
	}
	if err := engine.Deny("approval-2", "operator", "blocked"); err != nil {
...
func TestApprovalEngineRejectsUnknownApproval(t *testing.T) {
	engine := NewApprovalEngine()
	if err := engine.Approve("missing", "operator", "ok"); err == nil {
